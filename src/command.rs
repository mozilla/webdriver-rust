...
pub enum WebDriverCommand<T: WebDriverExtensionCommand> {
    NewSession(CapabilitiesParameters),
    DeleteSession,
    Get(GetParameters),
    GetCurrentUrl,
    GoBack,
...
impl <U: WebDriverExtensionRoute> WebDriverMessage<U> {
    ...
    pub fn from_http(match_type: Route<U>, params: &Captures, body: &str, requires_body: bool) -> WebDriverResult<WebDriverMessage<U>> {
        ...
        let command = match match_type {
            Route::NewSession => {
                let parameters: CapabilitiesParameters = try!(Parameters::from_json(&body_data));
...
#[derive(PartialEq)]
pub struct CapabilitiesParameters {
    pub desired: BTreeMap<String, Json>,
    pub required: BTreeMap<String, Json>
}

impl Parameters for CapabilitiesParameters {
    fn from_json(body: &Json) -> WebDriverResult<CapabilitiesParameters> {
