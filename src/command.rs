...
type Capabilities = BTreeMap<String, Json>;
...
impl SpecNewSessionParameters {
    fn validate<T: BrowserCapabilities>(&self,
        ...
        for (key, value) in capabilities.iter() {
            match &**key {
                ...
                "pageLoadStrategy" => {
                    try!(self.validate_page_load_strategy(value))
                }
                "proxy" => {
                    try!(self.validate_proxy(value))
                },
                "timeouts" => {
                    try!(self.validate_timeouts(value))
                },
                "unhandledPromptBehaviour" => {
                    try!(self.validate_unhandled_prompt_behaviour(value))
                }
                x => {
                    if !x.contains(":") {
                        return Err(WebDriverError::new(ErrorStatus::InvalidArgument,
    ...
    fn validate_page_load_strategy(&self, value: &Json) -> WebDriverResult<()> {
        match value {
            &Json::String(ref x) => {
                match &**x {
                    "normal" |
    ...
    fn validate_proxy(&self, proxy_value: &Json) -> WebDriverResult<()> {
        ...
        for (key, value) in obj.iter() {
            match &**key {
                ...
                "ftpProxy" => try!(self.validate_host_domain("ftpProxy", "ftp", obj, value)),
                "ftpProxyPort" => try!(self.validate_port("ftpProxyPort", value)),
                "httpProxy" => try!(self.validate_host_domain("httpProxy", "http", obj, value)),
                "httpProxyPort" => try!(self.validate_port("httpProxyPort", value)),
                "socksProxy" => try!(self.validate_host_domain("socksProxy", "ssh", obj, value)),
                "socksProxyPort" => try!(self.validate_port("socksProxyPort", value)),
    ...
    fn validate_port(&self, name: &str, value: &Json) -> WebDriverResult<()> {
        match value {
            &Json::I64(x) => {
                if x < 0 || x > 2^16 - 1 {
                    return Err(WebDriverError::new(
    ...
    fn validate_timeouts(&self, value: &Json) -> WebDriverResult<()> {
    ...
    fn validate_unhandled_prompt_behaviour(&self, value: &Json) -> WebDriverResult<()> {
