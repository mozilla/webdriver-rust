...
impl Parameters for WindowPositionParameters {
    fn from_json(body: &Json) -> WebDriverResult<WindowPositionParameters> {
        ...
        let xv = try_opt!(data.get("x"), ErrorStatus::InvalidArgument, "Missing 'x' parameters");
        let x = try_opt!(xv.as_u64(), ErrorStatus::InvalidArgument, "'x' is not a positive integer");
        let yv = try_opt!(data.get("y"), ErrorStatus::InvalidArgument, "Missing 'y' parameters");
        let y = try_opt!(yv.as_u64(), ErrorStatus::InvalidArgument, "'y' is not a positive integer");
